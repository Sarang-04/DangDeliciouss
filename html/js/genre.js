...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const pageGenre = (document.body.dataset.genre || "").toLowerCase().trim();
    ...
    fetch(sheetURL)
      ...
      .then(data => {
        ...
        if (!grid || !template) {
          console.error("Missing #recipe-grid or #recipe-template");
          return;
        }
  
        data.forEach(recipe => {
          const genre = (recipe["GENRE"] || "").toLowerCase().trim();
          if (genre === pageGenre) {
            const clone = template.content.cloneNode(true);
            clone.querySelector("img").src = recipe["IMAGE URL"] || "images/default.jpg";
            clone.querySelector(".title").textContent = recipe["TITLE"] || "Untitled";
